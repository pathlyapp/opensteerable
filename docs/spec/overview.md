...
**All 13 published packages** (TS protocol/harness/ui/pack-sdk/agent-shell/agent-shell-web on npm + Py protocol/harness/runtime/plugin-sdk/sidecar/egress-proxy/runtime-native on PyPI) are kept at the same version by `scripts/check_lockstep_versions.py` (CI-enforced on every tag push). The same gate also pins the unpublished Rust crates and the source-only TS `@steerable/agent-runtime` wrapper so a tag never carries mixed sources. Releases are operator-driven: `./scripts/release/bump_to.sh X.Y.Z` is the only writer of versions, and the lockstep gate in `.github/workflows/release.yml` refuses any tag whose source tree disagrees with the tag.
