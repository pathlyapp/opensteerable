...
describe('detectDeferredExecution', () => {
  describe('真实回归 case', () => {
    it('domain_tool "现在轮询结果。" 必须被识别为 deferred（句号结尾也算）', () => {
      // 这是 t_mp9vqnv7_1 那次抓到的真实文本——模型 emit 完 domain_tool_replay_card
  ...
  describe('意图词 + 开放收尾（rule ①/②）', () => {
    it('意图词 + 省略号 → deferred', () => {
      expect(detectDeferredExecution('我马上调用 domain_tool_get_task_result...')).toBe(true);
    });

    it('意图词 + 冒号收尾 → deferred', () => {
      expect(
        detectDeferredExecution('好的，接下来调用 domain_tool_replay_card 工具：'),
      ).toBe(true);
    });
  ...
  describe('避免误伤（已完成 / 总结类）', () => {
    ...
    it('过渡句在段落中间但最后一句是完成态 → 不算 deferred', () => {
      ...
      // 是完成态结论就放行。
      const text =
        '先调用了 domain_tool_test_connection，然后调用了 domain_tool_list_cards，已找到 2 个相关卡片。';
