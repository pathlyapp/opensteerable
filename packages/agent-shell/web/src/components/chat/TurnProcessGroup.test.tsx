...
const blocks: TurnBlock[] = [
  ...
  tool('domain_tool_get_config'),
  { type: 'reasoning', content: '再查天气' },
  tool('web_fetch'),
  { type: 'text', content: '本地 DomainTool 配置' },
];
...
describe('TurnProcessGroup', () => {
  it('collapses think+tool process after a finished summary, leaving the answer visible', () => {
    ...
    expect(screen.getByTestId('answer').textContent).toBe('本地 DomainTool 配置');
  });
  ...
  it('does not fold a tools-only turn that never produced a summary', () => {
    renderGroup({
      isStreaming: false,
      blocks: [tool('domain_tool_get_config')],
    });
    expect(screen.getByRole('button').getAttribute('aria-expanded')).toBe('true');
    expect(screen.getByTestId('tools-flow').textContent).toBe('domain_tool_get_config');
    expect(screen.queryByTestId('answer')).toBeNull();
  });
