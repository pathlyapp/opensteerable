...
class TestDeferredExecution:
    ...
    def test_intent_plus_ellipsis(self) -> None:
        assert detect_deferred_execution("我马上调用 domain_tool_get_task_result...")

    def test_intent_plus_trailing_colon(self) -> None:
        assert detect_deferred_execution("好的，接下来调用 domain_tool_replay_card 工具：")
    ...
    def test_transition_mid_paragraph_passes(self) -> None:
        text = "先调用了 domain_tool_test_connection，然后调用了 domain_tool_list_cards，已找到 2 个相关卡片。"
