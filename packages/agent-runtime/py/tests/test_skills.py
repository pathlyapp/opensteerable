...
""",
    ...
    write_skill(
        ...
        """---
name: domain_tool
displayName: 智能domain处理链
description: "DomainTool workflow guidance — connection, card scanning, replay."
priority: 600
conditions: [tool:domain_tool_list_cards]
match: all
---
...
def test_frontmatter_field_compat(skills_root: Path) -> None:
    ...
    assert set(by_name) == {"identity", "anti-deferred-execution", "local-exec", "domain_tool"}

    domain_tool = by_name["domain_tool"]
    assert domain_tool.display_name == "智能domain处理链"
    assert domain_tool.description == "DomainTool workflow guidance — connection, card scanning, replay."
    assert domain_tool.priority == 600
    assert domain_tool.conditions == ("tool:domain_tool_list_cards",)
    assert domain_tool.match == "all"
    assert domain_tool.dir_name == "90-domain_tool"
...
def test_layer_criterion(skills_root: Path) -> None:
    ...
    assert by_name["domain_tool"].layer == "catalog"
...
def test_get_resolves_aliases(skills_root: Path) -> None:
    provider = FilesystemSkillProvider([skills_root])
    assert provider.get("domain_tool") is not None
    assert provider.get("CFLOG") is not None  # case-insensitive
    assert provider.get("90-domain_tool") is not None  # dir name
    assert provider.get("智能domain处理链") is not None  # display name
...
def test_select_catalog_filters(skills_root: Path) -> None:
    ...
    # here (local-exec and domain_tool both carry conditions).
    assert select_catalog(provider.list()) == []
    # domain_tool tool active: domain_tool (match=all, single condition) listed.
    catalog = select_catalog(provider.list(), {"tool:domain_tool_list_cards"})
    assert [s.name for s in catalog] == ["domain_tool"]
    ...
    assert select_catalog(provider.list(), {"tool:domain_tool_list_cards"}, exclude=["CFLOG"]) == []
    assert select_catalog(provider.list(), {"tool:domain_tool_list_cards"}, exclude=["90-domain_tool"]) == []


def test_render_skill_catalog(skills_root: Path) -> None:
    provider = FilesystemSkillProvider([skills_root])
    catalog = select_catalog(provider.list(), {"tool:domain_tool_list_cards", "tool:local_read_file"})
    text = render_skill_catalog(catalog)
    assert "# Available skills" in text
    assert "`skill`" in text
    assert "- local-exec: Local shell / filesystem control" in text
    assert "- domain_tool(智能domain处理链): DomainTool workflow guidance" in text
...
async def test_skill_tool_roundtrip(skills_root: Path) -> None:
    provider = make_provider(
        [
            {"tool_calls": [tc("skill", {"name": "domain_tool"})]},
            {"content": "按技能执行完毕"},
        ]
    )
    ...
    events = await collect(
        loop.run(
            [LLMMessage(role="user", content="用 domain_tool 技能处理")],
    ...
    assert payload["data"] == {"skill": "domain_tool"}
    assert payload["message"].startswith('<skill_content name="domain_tool">\n')
    assert "# DomainTool" in payload["message"]
...
async def test_skill_tool_unknown_name_lists_available(skills_root: Path) -> None:
    ...
    assert "domain_tool" in tool_messages[0].content  # available list guides the retry
...
async def test_skill_tool_rejects_excluded(skills_root: Path) -> None:
    provider = make_provider(
        [{"tool_calls": [tc("skill", {"name": "domain_tool"})]}, {"content": "ok"}]
    )
    executor = SkillExecutor(
        RouterToolExecutor(ToolRouter()),
        FilesystemSkillProvider([skills_root]),
