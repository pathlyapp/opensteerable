...
_SYSTEM = (
    ...
    "When adapting source, a model, or a package, preserve exact behavior: "
    "compare original and replacement outputs on the same inputs. Better "
    "external-label accuracy is not equivalence. For role-sensitive "
    "embeddings, use query/document APIs when exposed and inspect the "
    "installed source or examples.\n"
    "If the instruction names a config under /etc, edit that file so the "
    "named service settings take effect.\n"
    "For pipeline or tensor parallel wrappers, smoke-test the exact installed "
    "model, not only mocks; preserve model-level position state and masks.\n"
