...
//   90-domain_tool      —— catalog 层（只进目录，按需加载）
...
const CFLOG = makeSkill({
  name: 'domain_tool',
  dirName: '90-domain_tool',
  content: 'CFLOG_EMBEDDED_GUIDANCE 用 domain_tool_scan_workspace 读取工区',
});
...
describe('prompt-builder / A6 分层披露', () => {
  ...
  it('eagerOnly 缺省时两层都进（非 CoreLoop 路径的兼容行为），按 dirName 排序', async () => {
    ...
    const domain_toolPos = prompt.indexOf('CFLOG_EMBEDDED_GUIDANCE');
    const userPos = prompt.indexOf('USER_SKILL_PROCEDURE');
    const otherPos = prompt.indexOf('OTHER_SKILL_CONTENT');
    for (const pos of [basePos, domain_toolPos, userPos, otherPos]) expect(pos).toBeGreaterThan(-1);
    // localeCompare：数字前缀排字母前缀之前
    expect(basePos).toBeLessThan(domain_toolPos);
    expect(userPos).toBeGreaterThan(domain_toolPos);
...
describe('prompt-builder / 智能体勾选的技能', () => {
  it('勾选的 catalog 层技能正文照样常驻——用户为该智能体点名了', async () => {
    const { prompt, modules } = await buildSystemPrompt({
      ignoreConditions: true,
      eagerOnly: true,
      pinnedSkillNames: ['90-domain_tool'],
    });
    expect(prompt).toContain('BASE_EAGER_CONTENT');
    expect(prompt).toContain('CFLOG_EMBEDDED_GUIDANCE');
    expect(prompt).not.toContain('USER_SKILL_PROCEDURE');
    expect(modules.map((m) => m.dirName)).toEqual(['00-base', '90-domain_tool']);
  });
