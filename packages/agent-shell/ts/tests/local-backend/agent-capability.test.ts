...
describe('mergeAgentCapabilities', () => {
  ...
  it('@提及多个智能体时取最宽松——叠加专家不该反而丢能力', () => {
    expect(
      mergeAgentCapabilities([
        makeAgent({ skillIds: ['90-domain_tool'], allowExternalSkills: false }),
        makeAgent({ skillIds: ['aa-geo'], allowExternalSkills: true, loadAllSkills: true }),
      ]),
    ).toEqual({
      pinnedSkills: ['90-domain_tool', 'aa-geo'],
      allowExternalSkills: true,
      loadAllSkills: true,
      toolPolicy: { mode: 'all', tools: [] },
    });
...
describe('resolveSkillExcludes', () => {
  const all = [
    ...
    makeSkill('90-domain_tool', 'domain_tool'),
    makeSkill('aa-geo', 'geo'),
  ];

  it('允许其他技能时只有模式级排除生效', () => {
    const capability = mergeAgentCapabilities([makeAgent({ skillIds: ['90-domain_tool'] })]);
